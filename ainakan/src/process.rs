...
use ainakan_sys::{AinakanSpawnOptions, _AinakanProcess};
use std::ffi::{CStr, CString};
use std::marker::PhantomData;

/// Process management in Ainakan.
pub struct Process<'a> {
    process_ptr: *mut _AinakanProcess,
    phantom: PhantomData<&'a _AinakanProcess>,
}

impl<'a> Process<'a> {
    pub(crate) fn from_raw(process_ptr: *mut _AinakanProcess) -> Process<'a> {
        Process {
            process_ptr,
            phantom: PhantomData,
        }
    ...
    pub fn get_name(&self) -> &str {
        let process_name =
            unsafe { CStr::from_ptr(ainakan_sys::ainakan_process_get_name(self.process_ptr) as _) };
    ...
    pub fn get_pid(&self) -> u32 {
        unsafe { ainakan_sys::ainakan_process_get_pid(self.process_ptr) }
    }
}

impl Drop for Process<'_> {
    fn drop(&mut self) {
        unsafe { ainakan_sys::ainakan_unref(self.process_ptr as _) }
    }
}
...
pub struct SpawnOptions<'a> {
    pub(crate) options_ptr: *mut AinakanSpawnOptions,
    phantom: PhantomData<&'a AinakanSpawnOptions>,
}

impl SpawnOptions<'_> {
    pub(crate) fn from_raw(options_ptr: *mut AinakanSpawnOptions) -> Self {
        Self {
            options_ptr,
            phantom: PhantomData,
        }
    ...
    pub fn new() -> Self {
        Self::from_raw(unsafe { ainakan_sys::ainakan_spawn_options_new() })
    }
        ...
        L: IntoIterator<Item = S>,
    {
        ...
        unsafe {
            ainakan_sys::ainakan_spawn_options_set_argv(
                self.options_ptr,
                arg_ptrs.as_mut_ptr(),
    ...
    pub fn cwd<S: AsRef<CStr>>(self, cwd: S) -> Self {
        unsafe {
            ainakan_sys::ainakan_spawn_options_set_cwd(
                self.options_ptr,
        ...
        M: IntoIterator<Item = (K, V)>,
    {
        ...
        unsafe {
            ainakan_sys::ainakan_spawn_options_set_env(
                self.options_ptr,
                env_ptrs.as_mut_ptr(),
        ...
        M: IntoIterator<Item = (K, V)>,
    {
        ...
        unsafe {
            ainakan_sys::ainakan_spawn_options_set_envp(
                self.options_ptr,
    ...
    pub fn stdio(self, stdio: SpawnStdio) -> Self {
        unsafe { ainakan_sys::ainakan_spawn_options_set_stdio(self.options_ptr, stdio as _) }
        self
    }
}
...
impl Drop for SpawnOptions<'_> {
    fn drop(&mut self) {
        unsafe { ainakan_sys::ainakan_unref(self.options_ptr as _) }
    }
}
