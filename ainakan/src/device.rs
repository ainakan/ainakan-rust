...
use ainakan_sys::_AinakanDevice;
...
/// Access to a Ainakan device.
pub struct Device<'a> {
    pub(crate) device_ptr: *mut _AinakanDevice,
    phantom: PhantomData<&'a _AinakanDevice>,
}

impl<'a> Device<'a> {
    pub(crate) fn from_raw(device_ptr: *mut _AinakanDevice) -> Device<'a> {
        Device {
            device_ptr,
            phantom: PhantomData,
        }
    ...
    pub fn get_name(&self) -> &str {
        let name =
            unsafe { CStr::from_ptr(ainakan_sys::ainakan_device_get_name(self.device_ptr) as _) };
        name.to_str().unwrap_or_default()
    }

    /// Returns the device's id.
    pub fn get_id(&self) -> &str {
        let id = unsafe { CStr::from_ptr(ainakan_sys::ainakan_device_get_id(self.device_ptr) as _) };
    ...
    pub fn get_type(&self) -> DeviceType {
        unsafe { ainakan_sys::ainakan_device_get_dtype(self.device_ptr).into() }
    }
    ...
    pub fn query_system_parameters(&self) -> Result<HashMap<String, Variant>> {
        ...
        let ht = unsafe {
            ainakan_sys::ainakan_device_query_system_parameters_sync(
                self.device_ptr,
                std::ptr::null_mut(),
                &mut error,
            )
        ...
        let mut iter: ainakan_sys::GHashTableIter =
            unsafe { std::mem::MaybeUninit::zeroed().assume_init() };
        unsafe { ainakan_sys::g_hash_table_iter_init(&mut iter, ht) };
        let size = unsafe { ainakan_sys::g_hash_table_size(ht) };
        let mut map = HashMap::with_capacity(size as usize);

        let mut key = std::ptr::null_mut();
        let mut val = std::ptr::null_mut();
        while (unsafe { ainakan_sys::g_hash_table_iter_next(&mut iter, &mut key, &mut val) }
            != ainakan_sys::FALSE as i32)
        {
    ...
    pub fn is_lost(&self) -> bool {
        unsafe { ainakan_sys::ainakan_device_is_lost(self.device_ptr) == 1 }
    }
        ...
        'a: 'b,
    {
        ...
        let processes_ptr = unsafe {
            ainakan_sys::ainakan_device_enumerate_processes_sync(
                self.device_ptr,
                std::ptr::null_mut(),
                std::ptr::null_mut(),
                &mut error,
        ...
        if error.is_null() {
            let num_processes = unsafe { ainakan_sys::ainakan_process_list_size(processes_ptr) };
            processes.reserve(num_processes as usize);

            for i in 0..num_processes {
                let process_ptr = unsafe { ainakan_sys::ainakan_process_list_get(processes_ptr, i) };
                let process = Process::from_raw(process_ptr);
                processes.push(process);
            }
        }

        unsafe { ainakan_sys::ainakan_unref(processes_ptr as _) };
        processes
    }
        ...
        'a: 'b,
    {
        let mut error: *mut ainakan_sys::GError = std::ptr::null_mut();
        let session = unsafe {
            ainakan_sys::ainakan_device_attach_sync(
                self.device_ptr,
                pid,
                std::ptr::null_mut(),
                std::ptr::null_mut(),
    ...
    pub fn spawn<S: AsRef<str>>(&mut self, program: S, options: &SpawnOptions) -> Result<u32> {
        ...
        let pid = unsafe {
            ainakan_sys::ainakan_device_spawn_sync(
                self.device_ptr,
                program.as_ptr(),
                options.options_ptr,
                std::ptr::null_mut(),
    ...
    pub fn resume(&self, pid: u32) -> Result<()> {
        let mut error: *mut ainakan_sys::GError = std::ptr::null_mut();
        unsafe {
            ainakan_sys::ainakan_device_resume_sync(
                self.device_ptr,
                pid,
                std::ptr::null_mut(),
                &mut error,
    ...
    pub fn kill(&mut self, pid: u32) -> Result<()> {
        let mut error: *mut ainakan_sys::GError = std::ptr::null_mut();
        unsafe {
            ainakan_sys::ainakan_device_kill_sync(
                self.device_ptr,
                pid,
                std::ptr::null_mut(),
                &mut error,
...
impl Drop for Device<'_> {
    fn drop(&mut self) {
        unsafe { ainakan_sys::ainakan_unref(self.device_ptr as _) }
    }
}
...
pub enum DeviceType {
    /// Local Ainakan device.
    Local = ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_LOCAL as _,

    /// Remote Ainakan device, connected via network
    Remote = ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_REMOTE as _,

    /// Device connected via USB
    USB = ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_USB as _,
}
...
impl From<u32> for DeviceType {
    fn from(value: u32) -> Self {
        match value {
            ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_LOCAL => Self::Local,
            ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_REMOTE => Self::Remote,
            ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_USB => Self::USB,
...
impl From<DeviceType> for ainakan_sys::AinakanDeviceType {
    fn from(value: DeviceType) -> Self {
        match value {
            DeviceType::Local => ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_LOCAL,
            DeviceType::Remote => ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_REMOTE,
            DeviceType::USB => ainakan_sys::AinakanDeviceType_AINAKAN_DEVICE_TYPE_USB,
        }
    }
}
