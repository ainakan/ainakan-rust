...
use ainakan_sys::{_AinakanInjector, g_bytes_new, g_bytes_unref};

/// Local library injector
///
/// Implements [Inject] to allow library injection into a target process.
pub struct Injector<'a> {
    injector_ptr: *mut _AinakanInjector,
    phantom: PhantomData<&'a _AinakanInjector>,
}

impl<'a> Injector<'a> {
    pub(crate) fn from_raw(injector_ptr: *mut _AinakanInjector) -> Injector<'a> {
        Injector {
            injector_ptr,
            phantom: PhantomData,
        }
    ...
    /// Create a new Injector using a `ainakan-helper` process.
    ///
    /// The `ainakan-helper` is a binary compiled into the Ainakan devkit, and is codesigned
    /// to allow debugging. It is spawned and injection is delegated to the helper.
    pub fn new() -> Self {
        Self::from_raw(unsafe { ainakan_sys::ainakan_injector_new() })
    }

    /// Create a new inprocess Injector
    ///
    /// Create a new Injector without using a `ainakan-helper` process.
    ///
    /// See [Injector::new] for details about the `ainakan-helper` process. Using an
    /// in_process injector may require the debugger process to be codesigned on some
    /// platforms.
    pub fn in_process() -> Self {
        Self::from_raw(unsafe { ainakan_sys::ainakan_injector_new_inprocess() })
    }
}
...
impl Drop for Injector<'_> {
    fn drop(&mut self) {
        unsafe {
            ainakan_sys::ainakan_injector_close_sync(
                self.injector_ptr,
                std::ptr::null_mut(),
                std::ptr::null_mut(),
            );
            ainakan_sys::ainakan_unref(self.injector_ptr as _)
        }
    }
}
...
/// ```no_run
///# use crate::ainakan::{Injector, Inject};
...
impl Inject for Injector<'_> {
        ...
        E: AsRef<str>,
    {
        ...
        let id = unsafe {
            ainakan_sys::ainakan_injector_inject_library_file_sync(
                self.injector_ptr,
                pid as ainakan_sys::guint,
                path.as_ptr() as *const ainakan_sys::gchar,
                entrypoint.as_ptr() as *const ainakan_sys::gchar,
                data.as_ptr() as *const ainakan_sys::gchar,
                std::ptr::null_mut(),
                &mut error,
            )
        };
        ...
        E: AsRef<str>,
    {
        ...
        let id = unsafe {
            ...
            let id = ainakan_sys::ainakan_injector_inject_library_blob_sync(
                self.injector_ptr,
                pid,
                g_blob,
                entrypoint.as_ptr() as *const ainakan_sys::gchar,
...
impl Inject for Device<'_> {
        ...
        E: AsRef<str>,
    {
        ...
        let id = unsafe {
            ainakan_sys::ainakan_device_inject_library_file_sync(
                self.device_ptr,
                pid as ainakan_sys::guint,
                path.as_ptr() as *const ainakan_sys::gchar,
                entrypoint.as_ptr() as *const ainakan_sys::gchar,
        ...
        E: AsRef<str>,
    {
        ...
        let id = unsafe {
            let g_blob = g_bytes_new(blob.as_ptr() as _, blob.len() as _);
            let id = ainakan_sys::ainakan_device_inject_library_blob_sync(
                self.device_ptr,
                pid,
                g_blob,
                entrypoint.as_ptr() as *const ainakan_sys::gchar,
