...
use ainakan_sys::_AinakanDeviceManager;
use std::ffi::CString;
...
pub struct DeviceManager<'a> {
    manager_ptr: *mut _AinakanDeviceManager,
    phantom: PhantomData<&'a _AinakanDeviceManager>,
}
...
impl<'a> DeviceManager<'a> {
    ...
    pub fn obtain<'b>(_ainakan: &'b Ainakan) -> Self
    where
        'b: 'a,
    {
        DeviceManager {
            manager_ptr: unsafe { ainakan_sys::ainakan_device_manager_new() },
            phantom: PhantomData,
        }
    }
        ...
        'a: 'b,
    {
        ...
        let devices_ptr = unsafe {
            ainakan_sys::ainakan_device_manager_enumerate_devices_sync(
                self.manager_ptr,
                std::ptr::null_mut(),
                &mut error,
            )
        ...
        if error.is_null() {
            let num_devices = unsafe { ainakan_sys::ainakan_device_list_size(devices_ptr) };
            devices.reserve(num_devices as usize);

            for i in 0..num_devices {
                let device =
                    Device::from_raw(unsafe { ainakan_sys::ainakan_device_list_get(devices_ptr, i) });
                devices.push(device);
            }
        }

        unsafe { ainakan_sys::ainakan_unref(devices_ptr as _) }
        devices
    }
    ...
    pub fn get_device_by_type(&'a self, r#type: DeviceType) -> Result<Device<'a>> {
        ...
        let device_ptr = unsafe {
            ainakan_sys::ainakan_device_manager_get_device_by_type_sync(
                self.manager_ptr,
                r#type.into(),
                0,
                std::ptr::null_mut(),
    ...
    pub fn get_remote_device(&'a self, host: &str) -> Result<Device<'a>> {
        ...
        let device_ptr = unsafe {
            ainakan_sys::ainakan_device_manager_add_remote_device_sync(
                self.manager_ptr,
                host_cstring.as_ptr(),
                std::ptr::null_mut(),
                std::ptr::null_mut(),
    ...
    pub fn get_device_by_id(&'a self, device_id: &str) -> Result<Device<'a>> {
        ...
        let device_ptr = unsafe {
            ainakan_sys::ainakan_device_manager_get_device_by_id_sync(
                self.manager_ptr,
                cstring.as_ptr(),
                0,
                std::ptr::null_mut(),
...
impl Drop for DeviceManager<'_> {
    fn drop(&mut self) {
        unsafe {
            ainakan_sys::ainakan_device_manager_close_sync(
                self.manager_ptr,
                std::ptr::null_mut(),
                std::ptr::null_mut(),
            );
            ainakan_sys::ainakan_unref(self.manager_ptr as _)
        }
    }
}
