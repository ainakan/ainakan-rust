...
use ainakan_sys::{
    AinakanScriptOptions, _AinakanScript, _GBytes, g_bytes_get_data, g_bytes_new, g_bytes_unref, gsize,
};
...
unsafe extern "C" fn call_on_message<I: ScriptHandler>(
    _script_ptr: *mut _AinakanScript,
    message: *const i8,
    data: *const ainakan_sys::_GBytes,
    ...
    match formatted_msg {
        Message::Send(ref msg) if msg.payload.r#type == "ainakan:rpc" => {
...
impl<'a> Script<'a> {
    pub(crate) fn from_raw(script_ptr: *mut _AinakanScript) -> Script<'a> {
    ...
    pub fn load(&self) -> Result<()> {
        ...
        unsafe { ainakan_sys::ainakan_script_load_sync(self.script_ptr, null_mut(), &mut error) };
    ...
    pub fn unload(&self) -> Result<()> {
        let mut error: *mut ainakan_sys::GError = std::ptr::null_mut();
        unsafe { ainakan_sys::ainakan_script_unload_sync(self.script_ptr, null_mut(), &mut error) };
    ...
    ///
    /// ```
    /// use ainakan::ScriptHandler;
    ///
    /// struct Handler;
    ///
    /// impl ScriptHandler for Handler {
    ///     fn on_message(&mut self, message: ainakan::Message, data: Option<Vec<u8>>) {
    ...
    pub fn handle_message<I: ScriptHandler + 'static>(&mut self, handler: I) -> Result<()> {
        ...
        unsafe {
            ...
            ainakan_sys::g_signal_connect_data(
    ...
    pub fn post<S: AsRef<str>>(&self, message: S, data: Option<&[u8]>) -> Result<()> {
        ...
        unsafe {
            ...
            ainakan_sys::ainakan_script_post(self.script_ptr as _, message.as_ptr() as _, g_data);
...
impl Drop for Script<'_> {
    fn drop(&mut self) {
        unsafe { ainakan_sys::ainakan_unref(self.script_ptr as _) }
    }
}

/// The JavaScript runtime of Ainakan.
pub enum ScriptRuntime {
    /// Default Ainakan runtime.
    Default,
    /// QuickJS runtime.
    QJS,
    /// Google V8 runtime.
...
impl From<ScriptRuntime> for ainakan_sys::AinakanScriptRuntime {
    fn from(runtime: ScriptRuntime) -> Self {
        match runtime {
            ScriptRuntime::Default => ainakan_sys::AinakanScriptRuntime_AINAKAN_SCRIPT_RUNTIME_DEFAULT,
            ScriptRuntime::QJS => ainakan_sys::AinakanScriptRuntime_AINAKAN_SCRIPT_RUNTIME_QJS,
            ScriptRuntime::V8 => ainakan_sys::AinakanScriptRuntime_AINAKAN_SCRIPT_RUNTIME_V8,
        }
    }
}

/// Represents options passed to the Ainakan script registrar.
pub struct ScriptOption {
    ptr: *mut AinakanScriptOptions,
}

impl ScriptOption {
    /// Create a new set of script options.
    pub fn new() -> Self {
        let ptr = unsafe { ainakan_sys::ainakan_script_options_new() };
        Self { ptr }
    }

    /// Get the name of the script.
    pub fn get_name(&self) -> &'static str {
        let name = unsafe { CStr::from_ptr(ainakan_sys::ainakan_script_options_get_name(self.ptr)) };
        name.to_str().unwrap_or_default()
    }

    /// Set the name of the script.
    pub fn set_name(self, name: &str) -> Self {
        unsafe { ainakan_sys::ainakan_script_options_set_name(self.ptr, name.as_ptr() as _) };
        self
    }

    /// Set the runtime of the script.
    pub fn set_runtime(self, runtime: ScriptRuntime) -> Self {
        unsafe { ainakan_sys::ainakan_script_options_set_runtime(self.ptr, runtime.into()) };
        self
    }

    pub(crate) fn as_mut_ptr(&mut self) -> *mut AinakanScriptOptions {
        self.ptr
    }
}
...
impl Drop for ScriptOption {
    fn drop(&mut self) {
        unsafe {
            ainakan_sys::g_clear_object(
                &mut self.ptr as *mut *mut ainakan_sys::_AinakanScriptOptions as _,
            )
        }
    }
}
