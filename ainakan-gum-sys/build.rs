...
fn main() {
    ...
    // We always use ainakan-gumjs.h for docs to not have to ship two big header files in this repo.
    ...
    let include_dir = {
        use ainakan_build::download_and_use_devkit;
        if cfg!(feature = "js") {
            download_and_use_devkit("gumjs", include_str!("AINAKAN_VERSION").trim())
        } else {
            download_and_use_devkit("gum", include_str!("AINAKAN_VERSION").trim())
        }
    };

    #[cfg(not(feature = "auto-download"))]
    if cfg!(feature = "js") {
        println!("cargo:rustc-link-lib=ainakan-gumjs");
    } else {
        println!("cargo:rustc-link-lib=ainakan-gum");
    }
    ...
    let bindings = if use_gum_js {
        bindings
            ...
            .header_contents("gum.h", "#include \"ainakan-gumjs.h\"")
    } else {
        bindings
            .clang_arg("-DUSE_GUM_JS=0")
            .header_contents("gum.h", "#include \"ainakan-gum.h\"")
    };
