...
fn download_and_use_devkit_internal(
    ...
    let devkit_name = format!("ainakan-{kind}-devkit-{version}-{os}-{target_arch}",);
    ...
    if !devkit_path.is_dir() {
        if !devkit_tar.is_file() {
            let ainakan_url = format!(
                "https://github.com/ainakan/ainakan/releases/download/{version}/{devkit_name}.tar.xz",
            );

            println!(
                "cargo:warning=Ainakan {} devkit not found, downloading from {}...",
                kind, &ainakan_url,
            );
            // Download devkit
            let mut resp =
                reqwest::blocking::get(&ainakan_url).expect("devkit download request failed");
    ...
    println!("cargo:rustc-link-lib=static=ainakan-{kind}");
