...
use {
    ...
    ainakan_gum_sys::{gboolean, gpointer},
};

#[cfg(not(feature = "std"))]
use alloc::{string::String, string::ToString, vec::Vec};
use cstr_core::CString;
use ainakan_gum_sys::GumThreadFlags_GUM_THREAD_FLAGS_ALL;

#[cfg(any(target_os = "linux", target_os = "freebsd"))]
extern "C" {
    pub fn _ainakan_g_get_home_dir() -> *const c_char;
    pub fn _ainakan_g_get_current_dir() -> *const c_char;
    pub fn _ainakan_g_get_tmp_dir() -> *const c_char;
}
...
pub struct Process<'a> {
    ...
    pub platform: Os,
    /// Property which can be `optional` or `required`, where the latter means Ainakan will avoid modifying
...
impl<'a> Process<'a> {
    ...
    pub fn current_dir(&self) -> String {
        unsafe {
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            let dir = _ainakan_g_get_current_dir();
            #[cfg(not(any(target_os = "linux", target_os = "freebsd")))]
    ...
    pub fn tmp_dir(&self) -> String {
        unsafe {
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            let dir = _ainakan_g_get_tmp_dir();
            #[cfg(not(any(target_os = "linux", target_os = "freebsd")))]
    ...
    pub fn home_dir(&self) -> String {
        unsafe {
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            let dir = _ainakan_g_get_home_dir();
            #[cfg(not(any(target_os = "linux", target_os = "freebsd")))]
    ...
    /// Enumerates process threads. May crash the application if called too early (if there are no non-ainakan threads)
    pub fn enumerate_threads(&self) -> Vec<Thread> {
        unsafe extern "C" fn enumerate_threads_callback(
            ...
            // subprojects/ainakan-gum/gum/gumthreadregistry.c
            1
        }
