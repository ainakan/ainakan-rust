...
) -> *mut ainakan_gum_sys::GumInvocationListener {
    let rust = ainakan_gum_sys::RustInvocationListenerVTable {
        user_data: invocation_listener as *mut _ as *mut c_void,
        on_enter: Some(call_on_enter::<I>),
        on_leave: Some(call_on_leave::<I>),
    };

    unsafe { ainakan_gum_sys::gum_rust_invocation_listener_new(rust) }
}
...
pub(crate) fn probe_listener_transform<I: ProbeListener>(
    ...
    let rust = ainakan_gum_sys::RustProbeListenerVTable {
        user_data: invocation_listener as *mut _ as *mut c_void,
        on_hit: Some(call_on_hit::<I>),
    };

    unsafe { ainakan_gum_sys::gum_rust_probe_listener_new(rust) }
}
