...
use {
    crate::NativePointer, core::ffi::c_void, ainakan_gum_sys as gum_sys,
...
unsafe extern "C" fn call_process<S: EventSink>(
    ...
    event: *const ainakan_gum_sys::GumEvent,
) {
    let event_sink: &mut S = &mut *(user_data as *mut S);
...
pub(crate) fn event_sink_transform<S: EventSink>(
    event_sink: &mut S,
) -> *mut ainakan_gum_sys::GumEventSink {
    let rust = ainakan_gum_sys::RustEventSinkVTable {
    ...
    unsafe { ainakan_gum_sys::gum_rust_event_sink_new(rust) }
}
