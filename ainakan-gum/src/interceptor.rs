...
impl Clone for Interceptor {
    fn clone(&self) -> Self {
        Interceptor {
            interceptor: unsafe {
                ainakan_gum_sys::g_object_ref(self.interceptor as *mut _) as *mut _
            },
...
impl Drop for Interceptor {
    fn drop(&mut self) {
        unsafe { ainakan_gum_sys::g_object_unref(self.interceptor as *mut _) }
    }
}
...
impl Drop for Listener {
    fn drop(&mut self) {
        ...
        unsafe { ainakan_gum_sys::g_object_unref(*ptr) }
    }
}
...
impl Clone for Listener {
    fn clone(&self) -> Self {
        let Self(NativePointer(ptr)) = self;
        Self(NativePointer(unsafe { ainakan_gum_sys::g_object_ref(*ptr) }))
    }
}
